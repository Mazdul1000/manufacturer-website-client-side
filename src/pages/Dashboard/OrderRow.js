...
const OrderRow = ({order, setOrder}) => {
    ...
    return (
        <div className='flex flex-col md:flex-row  py-5 px-5 md:justify-between justify-center items-center bg-accent rounded-xl shadow-lg '>
            ...
            {!paid && <label htmlFor="confirm-modal" onClick={()=> setOrder(order)} className="btn w-2/3 modal-button md:w-full btn-md bg-red-600 text-white border-0">Cancel Order</label>}
            {paid &&<label className="btn btn-md w-2/3 md:w-full bg-red-600 text-white border-0" disabled>Cancel Order</label>}
            </div>
        </div>
    );
};
